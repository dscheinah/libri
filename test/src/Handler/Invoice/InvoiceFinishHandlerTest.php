...
class InvoiceFinishHandlerTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->handler = new InvoiceFinishHandler(new ResponseHelper(), $this->repositoryMock);
    }

    public function testHandleSuccess(): void
    {
        $this->repositoryMock->expects($this->once())->method('finishInvoice')->with(1)->willReturn(true);
    ...
    {
        $this->repositoryMock->expects($this->once())->method('finishInvoice')->willReturn(false);
        /** @var Response $response */
        $response = $this->handler->handle((new ServerRequest())->withQueryParams(['id' => 1]));
